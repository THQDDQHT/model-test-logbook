...
class SqliteStore {
  ...
  static async open(dataDir) {
    ...
    `);
    const recordColumns = new Set(db.prepare('PRAGMA table_info(records)').all().map((row) => row.name));
    if (!recordColumns.has('reasoning_effort')) {
      db.exec("ALTER TABLE records ADD COLUMN reasoning_effort TEXT NOT NULL DEFAULT ''");
    }
    if (!recordColumns.has('harness')) {
      db.exec("ALTER TABLE records ADD COLUMN harness TEXT NOT NULL DEFAULT ''");
    }
  ...
  buildWhere(filter) {
    ...
    if (filter.q) {
      ...
      clauses.push(
        "(title LIKE ? ESCAPE '\\' OR prompt LIKE ? ESCAPE '\\' OR result LIKE ? ESCAPE '\\' OR model LIKE ? ESCAPE '\\' OR reasoning_effort LIKE ? ESCAPE '\\' OR harness LIKE ? ESCAPE '\\' OR tags LIKE ? ESCAPE '\\' OR batch_name LIKE ? ESCAPE '\\')",
      );
    ...
    if (filter.reasoning_effort && filter.reasoning_effort.length) {
      clauses.push(`reasoning_effort IN (${filter.reasoning_effort.map(() => '?').join(', ')})`);
      params.push(...filter.reasoning_effort);
    }
    if (filter.harness && filter.harness.length) {
      clauses.push(`harness IN (${filter.harness.map(() => '?').join(', ')})`);
  ...
  stats() {
    ...
    const reasoningRows = this.all(
      "SELECT reasoning_effort, COUNT(*) AS count FROM records WHERE reasoning_effort <> '' GROUP BY reasoning_effort ORDER BY count DESC",
    );
    const harnessRows = this.all("SELECT harness, COUNT(*) AS count FROM records WHERE harness <> '' GROUP BY harness ORDER BY count DESC");
    ...
    return finishStats({
      ...
      reasoning_efforts: reasoningRows.map((r) => ({ reasoning_effort: r.reasoning_effort, count: Number(r.count) })),
      harnesses: harnessRows.map((r) => ({ harness: r.harness, count: Number(r.count) })),
...
class JsonStore {
  ...
  load() {
    ...
    if (parsed && Array.isArray(parsed.records)) {
      this.state.records = parsed.records.map((record) => ({
        reasoning_effort: '',
        harness: '',
        ...record,
      }));
  ...
  stats() {
    ...
    for (const r of records) {
      ...
      if (r.reasoning_effort) reasoningMap.set(r.reasoning_effort, (reasoningMap.get(r.reasoning_effort) || 0) + 1);
      if (r.harness) harnessMap.set(r.harness, (harnessMap.get(r.harness) || 0) + 1);
    }
    ...
    return finishStats({
      ...
      reasoning_efforts: [...reasoningMap.entries()]
        .map(([reasoning_effort, count]) => ({ reasoning_effort, count }))
        .sort((a, b) => b.count - a.count),
      harnesses: [...harnessMap.entries()].map(([harness, count]) => ({ harness, count })).sort((a, b) => b.count - a.count),
      dbBytes: (() => {
        try {
...
function matchesFilter(r, f) {
  ...
  if (f.reasoning_effort && f.reasoning_effort.length && !f.reasoning_effort.includes(r.reasoning_effort)) return false;
  if (f.harness && f.harness.length && !f.harness.includes(r.harness)) return false;
